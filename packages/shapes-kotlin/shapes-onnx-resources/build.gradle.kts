...
// SwiftPM `ShapesONNXResources` product). The model files (shapes.onnx,
...
//     implementation("ai.desertant:shapes-onnx-resources")  // opt-in: bundle the model
//
...
tasks.jar {
    doFirst {
        ...
        val required = listOf("shapes.onnx", "shapes_meta.json")
...
mavenPublishing {
    ...
    coordinates("ai.desertant", "shapes-onnx-resources", version.toString())
    configure(JavaLibrary(javadocJar = JavadocJar.Empty(), sourcesJar = true))
    pom {
        name.set("Shapes ONNX resources")
